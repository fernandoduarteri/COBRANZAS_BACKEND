...
public class PrestamoDAO extends JPAEntity {
	...
	public void getPrestamo(ObjectReturn objReturn) {
		...
		String query = "select cc.venc_permitido, cc.ld_permitida, cc.id as id_cliente, p.id, p.observaciones, CEIL((p.monto * p.porcentaje_interes / 100) + p.monto + IFNULL(p.porcentaje_mora,0)) as monto, p.cuotas, CEIL((monto_cuota - IFNULL(sum(pp.monto_pagado),0))) as adeudado,"
				+ "               cc.cedula, c.id as id_cuota, c.numero, cc.nombre, cc.apellidos, IFNULL(c.monto_cuota - sum(pp.monto_pagado), c.monto_cuota) as monto_cuota, DATE_FORMAT(p.fecha_vencimiento, '%d-%m-%y') as fecha_vencimiento, monto_total from prestamo p"
				+ "             inner join cuota c"
				+ "                on c.prestamo = p.id"
				+ "                left join pago pp"
				+ "               on c.id = pp.cuota"
	...
	public void getPrestamoUnicoCliente(ObjectReturn objReturn) {
		int idPrestamo= (int) objReturn.getData();
		...
		String query = "select cc.venc_permitido, cc.ld_permitida, cc.id as id_cliente, p.id, p.observaciones, CEIL((p.monto * p.porcentaje_interes / 100) + p.monto + IFNULL(p.porcentaje_mora,0)) as monto, p.cuotas, CEIL((monto_cuota - IFNULL(sum(pp.monto_pagado),0))) as adeudado,"
				...
				+ "                inner join cliente cc"
				+ "               on cc.id = p.cliente"
				+ "                where p.id="+idPrestamo+ " and p.pagado = 0 and c.pagada = 0";
		try {
