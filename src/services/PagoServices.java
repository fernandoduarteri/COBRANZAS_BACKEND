		...
		try {
			if(pago.getMontoPagado()>pago.getCuotaBean().getMontoCuota()) {
				...
				if((montoTotal / cuotas ) == pago.getCuotaBean().getMontoCuota()) {
					...
					adeudoTotalPrestamo = (1+ ((int)resultPrestamo.get("cuotas") - (int)resultPrestamo.get("numero")))*pago.getCuotaBean().getMontoCuota();
					...
					adeudoTotalPrestamo = ((((int)resultPrestamo.get("cuotas") - (int)resultPrestamo.get("numero")))*cuota) + pago.getCuotaBean().getMontoCuota();
					
				}
				if(pago.getMontoPagado()>adeudoTotalPrestamo) {
					throw new Exception("El monto pagado no puede ser mahyor que el adeudo total del prestamo " + adeudoTotalPrestamo);
				}
			...
			objReturn.setData("Error al querer grabar los pagos");
